...
Routing is the way that we navigate to websites and web applications - it's how we communicate between computers and serves as a link between requests for resources and our code. The main difference between client-side and server-side routing is who looks at the URL. In server-side routing, the server is in charge of looking a tthe URL and sending views/HTML to the client. Because the server does this, each time we navigatet to a new URL, a new page is loaded. In client-side routing, the client is responsible for routing and looking at the URL- we don't have to make a new request to the server because all of the HTML and program already lives in the browser - it is faster and prevents the browser from refreshing.
...
axios & fetch libraries and componentDidMount
